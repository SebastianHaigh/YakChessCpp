...
int pop_1st_bit(uint64* bb) {
    ...
    // essentially we are XORing the most most significant half of the number with the least significant half
    // Although I am not sure if it is valid for 8-bits, for 8bits the results would be:
    //
    // 0000 0000 -> 0000
    // 0000 0001 -> 0001
    // 0000 0011 -> 0011
...
uint64 index_to_uint64(int index, int bits, uint64 m) {
    ...
    uint64 result = 0ULL;
    for (i = 0; i < bits; i++) {
        j = pop_1st_bit(&m); // get index of LSB square 0 - 63
